...
public class RobotContainer implements Sendable {
    ...
    /**
     * A chooser for autonomous commands. String in pair should be the path's name,
     * and null if no
     * path
     */
    ...
    private void configureDriver() {
        ...
        driverController
                ...
                .onFalse(
                        Conveyor.finishReceive(conveyor)
                                .andThen(
                                        new GoHomeSequence(
                                                intake, elevator, shooter, conveyor, intakeLimelight, false, false,
                                                true))
        ...
        SelectCommand<PrepState> driverLeftTriggerCommand = new SelectCommand<PrepState>(selectCommandMap,
                this::getAndClearPrepState);
    ...
    private void configureOperator() {
        /**
         * conveyor towards shooter -> right trigger -> DONE conveyor outtake -> left
         * trigger -> DONE
         *
         * <p>
         ...
         * -> DONE blue
         * subwoofer source side (same distance as normal subwoofer shot) -> right dpad
         * -> DONE
         *
         * <p>
         * red subwoofer amp side (same distance as normal subwoofer shot) -> right dpad
         * -> DONE red
         * subwoofer source side (same distance as normal subwoofer shot) -> left dpad
         * -> DONE
         *
         * <p>
         * under chain shot (diff headings for blue and red) -> y -> DONE amp shot (diff
         * headings for
         * blue and red) -> b -> DONE podium shot (diff headings for blue and red) -> x
         * -> DONE
         */
        BiFunction<Double, Double, Command> operatorPrepCreator = (Double distanceMeters,
                Double targetHeadingWhenRed) -> {
            return new InstantCommand(
                    () -> shooter.setShooterDistance(distanceMeters))
                    .andThen(
                            () -> {
        ...
        operatorController
                .povDown()
                .onTrue(
                        operatorPrepCreator.apply(ShooterCal.SUBWOOFER_SHOT_DISTANCE_METERS, 180.0));
        operatorController
                .povLeft()
                .onTrue(
                        operatorPrepCreator.apply(ShooterCal.SUBWOOFER_SHOT_DISTANCE_METERS, 120.0));

        operatorController
                .povRight()
                .onTrue(
                    operatorPrepCreator.apply(ShooterCal.SUBWOOFER_SHOT_DISTANCE_METERS, 240.0));

        operatorController
                .b()
                .onTrue(
                    operatorPrepCreator.apply(Units.inchesToMeters(142.0), 160.0));

        operatorController
                .x()
                .onTrue(
                    operatorPrepCreator.apply(Units.inchesToMeters(100.0), 202.0));

        operatorController
                .y()
                .onTrue(
                    operatorPrepCreator.apply(Units.inchesToMeters(174.0), 190.0));
