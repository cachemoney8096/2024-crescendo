...
public class CRTUtil {
  ...
  private CRTUtil(
      double mainGearRatioTerm,
      double secondaryGearRatioTerm,
      double mainGearCircumference,
      int requiredMaxRotations) {
    ...
    this.mainGearRatioTermSimplified =
        (double) (mainGearRatioTerm / gcd(mainGearRatioTerm, secondaryGearRatioTerm));
    this.secondaryGearRatioTermSimplfied =
        (double) (secondaryGearRatioTerm / gcd(mainGearRatioTerm, secondaryGearRatioTerm));
  }

  public static Optional<CRTUtil> init(
      double mainGearRatioTerm,
      double secondaryGearRatioTerm,
      double mainGearCircumference,
      int requiredMaxRotations,
    ...
    CRTUtil crtUtil =
        new CRTUtil(
            mainGearRatioTerm, secondaryGearRatioTerm, mainGearCircumference, requiredMaxRotations);
  ...
  private void initTable(double fractionalPrecisionDenominator) {
    for (int i = 0; i < requiredMaxRotations * fractionalPrecisionDenominator; i++) {
      ...
      double encoderTwo =
          mod(
              (elevatorPosition / mainGearCircumference)
                  * (mainGearRatioTerm / secondaryGearRatioTerm),
              1);
      table.add(
          new Pair<Pair<Double, Double>, Double>(
              new Pair<Double, Double>(encoderOne, encoderTwo), elevatorPosition));
    }
  }
  ...
  public double getAbsolutePosition(double mainEncoder, double secondaryEncoder) {
    ...
    Pair<Pair<Double, Double>, Double> inputPair =
        new Pair<Pair<Double, Double>, Double>(
            new Pair<Double, Double>(mainEncoder, secondaryEncoder), null);
    for (int i = 1; i < table.size(); i++) {
      if (getPositionPairErrorMargin(inputPair, table.get(i))
          < getPositionPairErrorMargin(fixedPair, inputPair)) {
  ...
  private double getPositionPairErrorMargin(
      Pair<Pair<Double, Double>, Double> one, Pair<Pair<Double, Double>, Double> two) {
    return Math.abs(one.getFirst().getFirst() - two.getFirst().getFirst())
        + Math.abs(one.getFirst().getSecond() - two.getFirst().getSecond());
  }
}
