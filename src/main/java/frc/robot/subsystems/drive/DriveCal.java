...
public class DriveCal {
    ...
    /**
     * Angular offset of the modules relative to the zeroing fixture in radians.
     * Ideally should be
    ...
    /**
     * Angular offsets of the modules relative to the chassis in radians. The
     * modules form an O when
     * fixtured, so they are iteratively 90 deg from each other.
     */
    public static final double FRONT_LEFT_CHASSIS_ANGULAR_OFFSET_RAD = SWERVE_FRONT_LEFT_ANGULAR_OFFSET_RAD
            - (3.0 * Math.PI / 4.0),
            FRONT_RIGHT_CHASSIS_ANGULAR_OFFSET_RAD = SWERVE_FRONT_RIGHT_ANGULAR_OFFSET_RAD + (3.0 * Math.PI / 4.0),
            BACK_LEFT_CHASSIS_ANGULAR_OFFSET_RAD = SWERVE_BACK_LEFT_ANGULAR_OFFSET_RAD - (1.0 * Math.PI / 4.0),
            BACK_RIGHT_CHASSIS_ANGULAR_OFFSET_RAD = SWERVE_BACK_RIGHT_ANGULAR_OFFSET_RAD + (1.0 * Math.PI / 4.0);

    /**
     * Controller on module speed for rotating to target, input degrees [-180,180],
     * output [0,1].
     */
    public static final PIDController ROTATE_TO_TARGET_PID_CONTROLLER = new PIDController(0.03, 0, 0.00);

    /**
     * The value for the keepHeading interpolation table based on normalized
     * translation velocity when
     * the robot is motionless (in xy). We want the lowest output to be 0.013 (that
     * is tuned),
     */
