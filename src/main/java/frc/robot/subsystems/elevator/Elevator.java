  ...
  private void controlPosition(double inputPositionInch) {
    ...
    elevatorDemandVoltsA =
        currentPIDController.calculate(
            leftMotorEncoderRel
                .getPosition()); // calculate the pid output based on the goal and the current
    // position
    ...
    if (prevTimestamp.isPresent()) {
      elevatorDemandVoltsB =
          currentFeedforward.calculate(
              ...
              timestamp
                  - prevTimestamp
                      .get()); // based on how velocity has changed, calculate a feedforward factor.
    ...
    elevatorDemandVoltsC =
        currentlyUsingNoteControl
            ? ElevatorCal.NOTE_SCORING_KS
            : ElevatorCal.CLIMBING_KS; // make sure we are applying enough voltage to move at all
    ...
    if (desiredPosition == ElevatorPosition.HOME && nearHome() && !currentlyUsingNoteControl) {
      voltageToSet =
          ElevatorCal
              .CLIMBING_KS; // if we are pretty close to home, we aren't going to execute a pid
